...
void swap_vbufs(void) {
    ...
    kmemcpy(antibuffer1, antibuffer0, (vbe_pitch / sizeof(uint32_t)) * vbe_height * sizeof(uint32_t));
...
retry:
    ...
    for (size_t i = 0; vid_modes[i] != 0xffff; i++) {
        ...
        if (vbe_mode_info.res_x == vbe_width
            ...
            framebuffer = (uint32_t *)((size_t)vbe_mode_info.framebuffer + MEM_PHYS_OFFSET);
