...
mkcontainer() {
  dd if=/dev/zero bs=512 count=0 seek=$(expr 2048 + ${SIZE} '*' 2048 + 33) "of=$OUTPUT"
  parted -s "$OUTPUT" mklabel gpt
  parted -s "$OUTPUT" mkpart primary 2048s $(expr 2048 + ${SIZE} '*' 2048 - 1)s
}

mkpartition() {
  dd if=/dev/zero bs=512 count=0 seek=$(expr ${SIZE} '*' 2048) of="$PARTITION"
...
insertpart() {
  dd if="$PARTITION" of="$OUTPUT" bs=1M seek=1 conv=notrunc
}
